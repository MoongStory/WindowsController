...
PVOID MOONG::WindowsController::old_value_ = NULL;

const std::string MOONG::WindowsController::get_active_directory_account()
{
	USES_CONVERSION;
...
const std::string MOONG::WindowsController::get_domain()
{
	USES_CONVERSION;
...
const std::string MOONG::WindowsController::get_primary_domain()
{
...
const std::string MOONG::WindowsController::get_sid_string()
{
	USES_CONVERSION;
...
const bool MOONG::WindowsController::get_user_name(OUT std::string& user_name)
{
	char buffer[256] = { 0 };
...
const std::string MOONG::WindowsController::get_windows_product_name()
{
...
const bool MOONG::WindowsController::wow64_redirection_enable(IN const BOOLEAN enable)
{
#if _MSC_VER > 1200
...
const bool MOONG::WindowsController::wow64_redirection_disable()
{
#if _MSC_VER > 1200
	return Wow64DisableWow64FsRedirection(&MOONG::WindowsController::old_value_) ? true : false;
#else
	return false;
#endif
}

const bool MOONG::WindowsController::wow64_redirection_revert()
{
#if _MSC_VER > 1200
	if (MOONG::WindowsController::old_value_ != NULL)
	{
		if (Wow64RevertWow64FsRedirection(MOONG::WindowsController::old_value_) == TRUE)
		{
			MOONG::WindowsController::old_value_ = NULL;
...
const int MOONG::WindowsController::messagebox_show_most_top(IN const std::string text, IN const std::string caption, IN const unsigned int type)
{
	return MessageBoxA(GetDesktopWindow(), text.c_str(), caption.c_str(), type | MB_SETFOREGROUND | MB_TOPMOST | MB_SYSTEMMODAL);
}

const bool MOONG::WindowsController::credential_write(const std::string& internet_or_network_address, const std::string& user_name, const std::string& password, const DWORD type/* = CRED_TYPE_GENERIC*/, const DWORD persist/* = CRED_PERSIST_LOCAL_MACHINE*/)
{
...
const bool MOONG::WindowsController::credential_delete(const std::string& internet_or_network_address, const DWORD type/* = CRED_TYPE_GENERIC*/)
{
	if (CRED_TYPE_DOMAIN_PASSWORD == type || CRED_TYPE_DOMAIN_CERTIFICATE == type)
	{
...
const std::string MOONG::WindowsController::find_execute_program(IN const std::string extension)
{
